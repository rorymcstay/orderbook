...
public:
  explicit Params(std::string str_) : _data(), _paramString(std::move(str_)) {}
  std::string &operator[](const std::string &key_) { return _data[key_]; }
  std::string &at(const std::string &key_) {
    try {
      return _data.at(key_);
    } catch (std::exception &ex) {
      ERROR("Missing mandatory parameter! " << LOG_VAR(key_)
                                            << LOG_VAR(_paramString));
      throw ex;
    }
  }
...
struct EnvMessage {
  ...
  explicit EnvMessage(Params params_) {
    ...
    auto temporder = std::make_shared<Order>(side, qty, price);
    if (params_["Type"] == "NewOrder") {
      ...
      execReport = std::make_shared<ExecReport>(temporder, execType);
    } else {
      throw std::runtime_error("Unknown Type of test message " +
                               params_["Type"]);
    }
  }
};
...
public:
  explicit TestEnv(const std::string &symbol_, double closePrice_)
      : _orderBook(std::make_shared<OrderBook>(symbol_, closePrice_)) {
    _orderBook->start();
  }
  ~TestEnv() { _orderBook->stop(); }

  const OrderBook::Ptr &orderBook() const { return _orderBook; }

private:
  OrderBook::Ptr _orderBook;
...
public:
  // operator in <<
  EnvMessage operator<<(const std::string &str_) {
    Params params(str_);
    messageFrom(str_, params);
    auto message = EnvMessage(params);
  ...
  void operator>>(const std::string &str_) {
    ...
    if (str_.find("NONE") != std::string::npos) {
      ...
      if (message != nullptr) {
        FAIL() << str_
               << " Unmatched event: " << LOG_NVP("OrderID", message->orderID())
               << LOG_NVP("Text", message->text())
               << LOG_NVP("ExecType", message->execType())
               << LOG_NVP("Price", message->price())
               << LOG_NVP("OrdQty", message->ordQty())
               << LOG_NVP("CumQty", message->cumQty())
               << LOG_NVP("LastQty", message->lastQty())
               << LOG_NVP("LastQty", message->lastPrice())
               << LOG_NVP("OrdStatus", message->ordStatus());
      }
      return;
    }
